...
* Sistema centralizzato di esportazione Excel
...
Queste sono descrizioni dettagliate  delle funzioni nascoste all'interno della scheda US-USM. Una di queste caratteristiche è la SQLPromptDialog, che è una finestra di dialogo per la gestione delle query SQL.

SQLPromptDialog
~~~~~~~~~~~~~~~~~~~

SQLPromptDialog, sviluppata con PyQt5, offre funzionalità relative alle query SQL attraverso vari pulsanti:
...
- "Create  Graph"
    ...
    .. note:: Chiave API
        ...
        Una chiave API è un codice unico usato per identificare l'interazione con un'applicazione o un servizio.
        In SQLPromptDialog, è necessaria una chiave API che puoi recuperare con una sottoscrizione a
        `Tex2SQL <https://text2sql.ai>`_ per eseguire query SQL.

Gestione del Prompt
~~~~~~~~~~~~~~~~~~~

di Seguito sono descritti i vari pulsanti e le loro funzionalità:

Pulsante Start Query
+++++++++++++++++++++

Dopo l'apertura di SQLPromptDialog, è disponibile un campo per inserire la query SQL. Supponendo una tabella chiamata "Prodotti" e si desidera selezionare tutti i dati, è possibile digitare "SELECT * FROM Prodotti;" nel campo. Dopo di che, cliccare su "Esegui Query" per procedere.

Pulsante Explain Query
+++++++++++++++++++++++

Se è necessario ottenere una spiegazione sulla query, si può digitare la query SQL nel campo di input e fatto ciò, cliccare su "Spiega Query". Ciò fornirà una spiegazione della query SQL inserita.

Pulsante Export Excel
+++++++++++++++++++++++++

A seguito dell'esecuzione di una query SQL con il pulsante "Esegui Query", è possibile esportare i risultati su un file Excel facendo clic su "Esporta su Excel".

Pulsante Create Graph
++++++++++++++++++++++

Per generare un grafico basato sui risultati dell'ultima query SQL eseguita, eseguire la query come spiegato precedentemente, e poi fare clic su "Crea Grafico".

È importante notare che ogni pulsante diventa attivo solo sotto determinate condizioni. Ad esempio, i pulsanti "Esporta su Excel" e "Crea Grafico" saranno attivi solo dopo l'esecuzione di una query SQL. Il pulsante "Spiega Query" sarà attivo solo quando viene inserito un input di query SQL valido.
